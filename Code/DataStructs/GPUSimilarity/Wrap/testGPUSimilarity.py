...
class GPUSimilarityTests(unittest.TestCase):

  def setUp(self):
    self.rng = random.Random(0xC0FFEE)

  def test_returns_2d_double_array_of_expected_shape(self):
  ...
  def test_selected_backend_string_is_known(self):
    self.assertIn(DataStructs.SelectedBackend(), ("cuda", "cpu"))

  def test_cuda_available_reports_consistently(self):
    available = DataStructs.CudaAvailable()
    description = DataStructs.CudaDeviceDescription()
    if available:
      self.assertNotEqual(description, "")
    else:
      self.assertEqual(description, "")

  def test_cuda_path_matches_cpu_when_available(self):
    if not DataStructs.CudaAvailable():
      self.skipTest("no CUDA device available")
    probes = _random_fps(13, 2048, self.rng)
    targets = _random_fps(17, 2048, self.rng)
    cpu = DataStructs.BulkTanimotoMatrixCpu(probes, targets)
    gpu = DataStructs.BulkTanimotoMatrixCuda(probes, targets)
